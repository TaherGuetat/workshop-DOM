...
for (let i = 0; i < btnadd.length; i++) {
  btnadd[i].addEventListener("click", function () {
    if (tab[i] == true) {
      ...
      panier.insertBefore(divprod, total.parentElement);
      ...
      pricet.innerHTML = price.innerHTML;
      ...
      btnmoins.addEventListener("click", decreament);
      //total
      total.innerHTML = parseInt(total.innerHTML) + parseInt(price.innerHTML);
      //delete
      btndel.addEventListener('click',function(){
divprod.remove();
total.innerHTML = parseInt(total.innerHTML) - parseInt(pricet.innerHTML);      });
    }
  });
}
function increament(e) {
  ...
  var pricettag = e.target.parentElement.nextElementSibling;
  var prixunitairetag = parseInt(
    e.target.parentElement.previousElementSibling.innerHTML
  );
  console.log(prixunitairetag);
  var pricetvalue = parseInt(pricettag.innerHTML);
  // console.log(parseInt(pricetvalue))
  pricetvalue = pricetvalue + prixunitairetag;
  pricettag.innerHTML = pricetvalue + "DT";
  total.innerHTML = parseInt(total.innerHTML) + prixunitairetag;
}
...
//       : parseInt(total.innerHTML) - prixunitairetag;
if (quantityvalue>1) {
    quantityvalue--
    quantity.innerHTML = quantityvalue;
    pricettag.innerHTML = pricetvalue - prixunitairetag + "DT";

    total.innerHTML=parseInt(total.innerHTML) - prixunitairetag+ "DT";
}
}
